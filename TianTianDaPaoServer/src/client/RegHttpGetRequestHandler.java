...
public class RegHttpGetRequestHandler extends AbstractHttpRequestHandler {

	private static Logger logger = Logger.getLogger(RegHttpGetRequestHandler.class);
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			e.printStackTrace();
			logger.info("注册请求出错, ip地址：" + channel.getRemoteAddress() + "    uri = " + uri);
			try {
