...
//3K客户端直冲
public class DaPaoRechargeCommon3KCheck extends Check {
	public DaPaoRechargeCommon3KCheck() {
		super();
	}
	...
	public JSONObject check(Map<String, String> params, Channel channel) {
		...
		try {
			...
			if(selectStateMap==null||(boolean)selectStateMap.get("state"))
			{
				...
				U.infoQueue("3K游戏充值回调失败：订单号不存在或订单已完成"
			...
			if(selectMap==null)
			{
				...
				U.infoQueue("3K游戏充值回调失败：uid不存在或格式错误"
						+ channel.getRemoteAddress().toString());
				return jsonObject;
			}
			value=value/100;
			...
			selectMap.put("channelID", 10);
			selectMap.put("uid", order);
			loginDao.updateRMBrecord(selectMap);
			sqlSession.commit();
			selectMap = loginDao.selectRechargeByUID(params);
			U.infoQueue("3K游戏充值回调成功：钻石增加"+value/100 * MoneyAppendConfig.getInstance().ratioDiamond
					+ channel.getRemoteAddress().toString());
