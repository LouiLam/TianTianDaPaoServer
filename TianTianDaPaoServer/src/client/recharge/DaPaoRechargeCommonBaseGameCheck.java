...
//指易付
public class DaPaoRechargeCommonBaseGameCheck extends Check {
	public DaPaoRechargeCommonBaseGameCheck() {
		super();
	}
	...
	public JSONObject check(Map<String, String> params, Channel channel) {
		...
		try {
			...
			Map<Object, Object> selectMapnew=	loginDao.selectUIDByOrder(params);
			
//			String uid=params.get("uid");
			String uid=(String) selectMapnew.get("id_uid");
			...
			if(selectStateMap==null||(boolean)selectStateMap.get("state"))
			{
				...
				U.infoQueue("话付宝游戏充值回调失败：订单号不存在或订单已完成"
			...
			if(selectMap==null)
			{
				...
				U.infoQueue("话付宝游戏充值回调失败：uid不存在或格式错误"
						+ channel.getRemoteAddress().toString());
				return jsonObject;
			}
			if(!We.equals(sign))
			{
				...
				U.infoQueue("话付宝游戏充值回调失败：加密MD5值和原始匹配不合法 对方sign="+sign+"我方md5Result="+We
						+ "        "+channel.getRemoteAddress().toString());
				return jsonObject;
			}
			value=value/100;
			...
			selectMap.put("channelID", 8);
			selectMap.put("uid", order);
			loginDao.updateRMBrecord(selectMap);
			sqlSession.commit();
			U.infoQueue("话付宝游戏充值回调成功：钻石增加"+value * MoneyAppendConfig.getInstance().ratioDiamond
					+ channel.getRemoteAddress().toString());
