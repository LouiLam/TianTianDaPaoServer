...
//指易付
public class DaPaoRechargeCommonHuaWeiCheck extends Check {
	public DaPaoRechargeCommonHuaWeiCheck() {
		super();
	}
	...
	public JSONObject check(Map<String, String> params, Channel channel) {
		...
		try {
			...
			if(selectStateMap==null||(boolean)selectStateMap.get("state"))
			{
				...
				U.infoQueue("华为游戏充值回调失败：订单号不存在或订单已完成"
			...
			if(selectMap==null)
			{
				...
				U.infoQueue("华为游戏充值回调失败：uid不存在或格式错误"
						+ channel.getRemoteAddress().toString());
				return jsonObject;
			}
			if(!We.equals(sign))
			{
				...
				U.infoQueue("华为游戏充值回调失败：加密MD5值和原始匹配不合法 对方sign="+sign+"我方md5Result="+We
						+ "        "+channel.getRemoteAddress().toString());
				return jsonObject;
			}
