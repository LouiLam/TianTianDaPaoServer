...
public class JJC_PKHttpGetRequestHandler extends AbstractHttpRequestHandler {

	private static Logger logger = Logger.getLogger(JJC_PKHttpGetRequestHandler.class);
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			e.printStackTrace();
			logger.info("竞技场挑战请求出错, ip地址：" + channel.getRemoteAddress() + "    uri = " + uri);
			try {
