 ...
 *
 */
public class GetNoticeHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			jsonObj.put("notice", GetNoticeConfigMgr.getInstance().getNoticeList);
			//返回
			sendResponse(jsonObj.toString(), channel);
			U.infoQueue("获取公告请求成功： "+channel.getRemoteAddress().toString());
		}catch(Exception e){	
			e.printStackTrace();
