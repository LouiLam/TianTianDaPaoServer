 ...
 *
 */
public class RechargeCommonHuaFuBaoHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String parametersString, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("36").check(paramClone,channel);
			//返回
			sendResponse("0", channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("话付宝回调充值请求发生异常： "+e.getMessage()+"ip地址："
