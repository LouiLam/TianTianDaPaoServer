 ...
 *
 */
public class GetTiliInfoHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("10").check(paramClone,channel);
			//返回
			sendResponse(jsonObj.toString(), channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("客户端获取体力信息请求发生异常： "+e.getMessage()+"ip地址："
