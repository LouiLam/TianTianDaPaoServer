 ...
 *
 */
public class TestAddDiamondHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("29").check(paramClone,channel);
			//返回
			sendResponse(jsonObj.toString(), channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("游戏测试增加钻石发生异常： "+e.getMessage()+"ip地址："
