 ...
 *
 */
public class GenerateOrdersHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("26").check(paramClone,channel);
			
			//返回
			sendResponse(jsonObj.toString(), channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("生成订单请求发生异常： "+e.getMessage()+"ip地址："
