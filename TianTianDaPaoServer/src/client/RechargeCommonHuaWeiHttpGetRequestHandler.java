 ...
 *
 */
public class RechargeCommonHuaWeiHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String parametersString, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("35").check(paramClone,channel);
			//返回
			sendResponse("0", channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("华为回调充值请求发生异常： "+e.getMessage()+"ip地址："
