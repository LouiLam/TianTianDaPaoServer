 ...
 *
 */
public class RechargeTelecomPaoKuHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String parametersString, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("37").check(paramClone,channel);
			//返回
			sendResponse(jsonObj.toString(), channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("联通跑酷充值请求发生异常： "+e.getMessage()+"ip地址："
