...
public class DaPaoChargeExchangeCheck extends Check {
	...
	public JSONObject check(Map<String, String> params, Channel channel) {
		...
		try {
			...
			jsonObject.put(Constant.RET, Constant.RET_CHARGE_EXCHANGE_SUCCESS);
			jsonObject.put(Constant.MSG,
					ConfigFactory.getRetMsg(Constant.RET_CHARGE_EXCHANGE_SUCCESS));
			...
			else
			{
				...
				jsonObject.put(Constant.MSG,
						ConfigFactory.getRetMsg(Constant.RET_CHARGE_EXCHANGE_SUCCESS));
				U.infoQueue("id:" + selectMap.get("id") + "兑换话费点请求失败，第三方平台错误码"+result
						+ channel.getRemoteAddress().toString());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int  httpGet() {
		String decodeString="partner=11588&out_trade_id=dapao"+count+"&mobile="+phone+"&value="+value+"&type=0"+"&notify_url="+notify_url+"&"+KEY;
		StringBuffer url=new StringBuffer("http://api.99huafei.com/mobile/direct.aspx?partner=11588&out_trade_id=dapao"+count);
...
//				System.out.println(strResult);
				Document documentCharge = DocumentHelper.parseText(strResult);
//				<return>
//				  <result>0</result> 
//				  <sign>163500DE4E885A9468644423A3433C58</sign> 
//				  </return>
				int result=Integer.parseInt(documentCharge.selectSingleNode("/return/result").getText());
				return result;
			}
			else
			{
