 ...
 * 
 */
public class ConfigInfoHttpGetRequestHandler extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try {
			...
			JSONObject jsonObj = ConfigFactory.getCheck("14").check(paramClone,
					channel);
			// 返回
			sendResponse(jsonObj.toString(), channel);
		} catch (Exception e) {
			e.printStackTrace();
			U.infoQueue("客户端配置信息请求发生异常： " + e.getMessage() + "ip地址："
