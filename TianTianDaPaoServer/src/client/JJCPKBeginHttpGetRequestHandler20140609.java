 ...
 *
 */
public class JJCPKBeginHttpGetRequestHandler20140609 extends AbstractHttpRequestHandler {
	...
	protected void handle(String uri, Channel channel) {
		try{
			...
			JSONObject jsonObj = ConfigFactory
					.getCheck("32").check(paramClone,channel);
			//返回
			sendResponse(jsonObj.toString(), channel);
		}catch(Exception e){	
			e.printStackTrace();
			U.infoQueue("竞技场挑战开始请求发生异常： "+e.getMessage()+"ip地址："
